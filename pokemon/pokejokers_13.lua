...
local turtwig={
  name = "turtwig", 
  pos = {x = 0, y = 0},
  config = {extra = {h_size = 1, card_threshold = 30 or #G.deck.cards - 12, has_fewer_cards = false, interest_rate = 1, interest_cap = 5}, evo_rqmt = 40},
  loc_vars = function(self, info_queue, center)
    type_tooltip(self, info_queue, center)
    return {vars = {center.ability.extra.h_size, center.ability.extra.card_threshold, center.ability.extra.interest_rate, self.config.evo_rqmt}}
  end,
  rarity = 2, 
  cost = 6, 
  stage = "Basic", 
  ...
  if context.setting_blind then
   G.GAME.interest_amount = G.GAME.interest_amount - card.ability.extra.interest_rate
      if not card.ability.extra.has_fewer_cards and #G.playing_cards - G.GAME.starting_deck_size <= 30  then
		card.ability.extra.has_fewer_cards = true
	  elseif #G.playing_cards - G.GAME.starting_deck_size > 30 then
		card.ability.extra.has_fewer_cards = false
	  end
	  if context.end_of_round and not context.repetition and not context.individual and not context.blueprint and card.ability.extra.has_fewer_cards then
		card_eval_status_text(card, 'extra', nil, nil, nil, {message = localize('poke_turtwig_ex')})
		card.ability.extra.has_fewer_cards  = false
	  elseif not card.ability.extra.has_fewer_cards then
		G.GAME.interest_amount = G.GAME.interest_amount - card.ability.extra.interest_rate
